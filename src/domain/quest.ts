...
import { clonePlain, clamp } from './math.ts';
...
import type { ActorState, QuestState } from './types.ts';

const { questCatalog } = DATA;
type QuestRewardValue = number | [number, number];
type QuestReward = Record<string, QuestRewardValue>;

function rewardNumber(value: QuestRewardValue | undefined): number {
...
export function updateQuestProgress(dt: number) {
  ensureQuestState();
