...
import { awardWeaponDefeatProficiency, awardWeaponHitProficiency, proficiencyDamageMultiplier } from "../../proficiency.ts";
...
export function resolveArrowHit(arrow: ArrowProjectile, target: ActorState) {
  ...
  const variance = 3;
  const baseDmg = Math.max(1, Math.ceil((state.player.atk + Math.floor(Math.random() * variance)) * arrow.damageScale * (critical ? 1.2 : 1) * proficiencyDamageMultiplier('bow')));
