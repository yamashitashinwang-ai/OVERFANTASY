...
import type { ActorState, GearCatalogItem, RegionTable } from '../types.ts';

const { gearCatalog, regions } = DATA;

export const playableRaces = ['人类', '精灵', '矮人'];

export const raceStartPoints: Record<string, { scene: string; x: number; y: number }> = {
...
export function raceDamageMultiplier(kind: string, weapon: GearCatalogItem = currentWeapon()): number {
...
export function raceMoveSpeedMultiplier(): number {
  return state.player?.race === '矮人' ? 0.95 : 1;
}

export function raceDefenseMultiplier(): number {
  return state.player?.race === '矮人' ? 1.1 : 1;
}

export function raceStaminaRegenMultiplier(): number {
  return state.player?.race === '精灵' ? 1.2 : 1;
}

export function applyRaceFinalAmount(amount: number, multiplier: number, min = 1): number {
  return Math.max(min, Math.round(amount * multiplier));
}

/** Tilt the starting region relations for elf/dwarf characters (cross-grudge). */
export function applyRaceInitialRegionRelations(race: string, regionMap: RegionTable = regions) {
...
export function applyRaceStartingLoadout(race: string) {
...
export function hostileRaceDialogue(npc: ActorState | null | undefined): boolean {
