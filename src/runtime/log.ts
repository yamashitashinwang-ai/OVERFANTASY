...
//      `window.__dumpLogs()` — the dev hook in scenes/Game.ts wires this up.
...
function bufferPush(ns: string, args: LogArg[]) {
...
function getLogger(ns: string) {
  if (!cache.has(ns)) {
    ...
    const wrapped = (...args: LogArg[]) => {
      bufferPush(ns, args);
      const debugFn = fn as (...items: LogArg[]) => void;
      debugFn(...args);
    };
...
// ─── Public API ────────────────────────────────────────────────────────────
// Usage:
//   import { log, NS } from '../runtime/log.ts';
