...
import type { GameState, PlayerState } from '../domain/types.ts';
 ...
 */
export function syncRegistry(state: GameState) {
  if (!scene) return;
  const reg = scene.registry;
...
export function getPlayerStat(key: TrackedPlayerKey) {
  return scene?.registry.get(`player.${key}`);
}

/** Subscribe to a single key. Returns an unsubscribe function. */
export function onPlayerStat(key: TrackedPlayerKey, fn: (...args: unknown[]) => void) {
  if (!scene) return () => {};
