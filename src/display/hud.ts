...
import { tile, W, H } from '../runtime/constants.ts';
import { clamp, formatNumber } from '../domain/math.ts';
import { currentWeapon, refreshCombatStats, hasPathosEffect } from '../domain/combat/weapon.ts';
...
export function syncHudDisplay() {
  ...
  D.hudBarsGfx.clear();
  const drawBar = (value: number, max: number, barY: number, color: number) => {
