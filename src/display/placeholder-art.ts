...
const playerRigTextureParts: PlayerRigTexturePart[] = ['head', 'torso', 'upperArm', 'forearm', 'thigh', 'shin', 'hand', 'foot'];
...
function drawRigPartTexture(g: Graphics, part: PlayerRigTexturePart, monster = false) {
  const skin = monster ? 0xb482ff : 0xf1c49a;
...
export function ensurePlaceholderArt(scene: Phaser.Scene) {
  for (const monsterForm of [false, true]) {
    ...
    for (const part of playerRigTextureParts) {
      const size = part === 'head' ? { w: 24, h: 24 }
