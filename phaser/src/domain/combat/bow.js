...
// setBowCharge() because the slot is a `let` in scenes/Game.js.

import { state, tile, flyingArrows } from '../../scenes/Game.js';
import { clamp, rand, segmentPointDistance } from '../math.js';
import { currentWeapon } from './weapon.js';
import { raceDamageMultiplier, applyRaceFinalAmount } from './race.js';
import { canUseWorldActions, attackEntityFilter } from './targeting.js';
import { addPickup } from '../world.js';
import { markHitReaction, defeatEntity } from './damage.js';
import {
  getBowCharge, setBowCharge,
  getHitStopTimer, setHitStopTimer,
  playerAimAngle, log, toast, renderStats
} from '../../scenes/Game.js';

export function isBowWeapon(weapon = currentWeapon()) {
  return weapon?.type === '弓';
}

export function bowChargeProgress() {
  return clamp((getBowCharge()?.time || 0) / 1.25, 0, 1);
}

export function bowShotStats(weapon, charge) {
  const minRange = 3.5;
...
export function beginBowCharge() {
...
export function releaseBowCharge() {
...
export function fireArrow(charge, rushedAttack = false) {
  const weapon = currentWeapon();
...
export function addArrowPickup(x, y) {
  addPickup("arrow", "箭", x, y, "#dbe4ea");
}

export function dropEmbeddedArrows(e) {
...
export function resolveArrowHit(arrow, target) {
...
export function updateFlyingArrows(dt) {
