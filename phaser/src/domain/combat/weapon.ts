...
import type { GearCatalogItem, GearMod, GearSlot } from '../types.ts';

const { gearCatalog } = DATA;

export function currentWeapon(): GearCatalogItem {
...
export function equippedGear(): GearCatalogItem[] {
  return Object.values(state.player.gear)
    .filter(Boolean)
    .map(id => gearCatalog[id as string])
    .filter(Boolean);
}

export function gearModList(gearId: string | null | undefined): GearMod[] {
  if (!gearId) return [];
  return state.player.gearMods[gearId] || [];
}

export function equippedModList(): GearMod[] {
  return Object.values(state.player.gear)
    .filter(Boolean)
    .flatMap(id => gearModList(id));
}

export function totalAtk(): number {
...
export function totalDef(): number {
...
export function hasPathosEffect(): boolean {
...
export function slotName(slot: GearSlot | string): string {
  return {
    weapon: '武器',
    head: '头',
    body: '衣服',
...
export function gearLabel(id: string | null | undefined): string {
  if (!id) return '';
