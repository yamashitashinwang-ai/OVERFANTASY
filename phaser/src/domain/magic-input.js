...
export function normalizeMagicTerm(value) {
  return String(value || '')
    .normalize('NFKC')
    .toLowerCase()
...
export function magicByInput(input) {
...
export function forbiddenMagicByInput(input) {
  const normalized = normalizeMagicTerm(input);
  if (!normalized) return null;
 ...
 */
export function isNearMagicName(input) {
  const normalized = normalizeMagicTerm(input);
