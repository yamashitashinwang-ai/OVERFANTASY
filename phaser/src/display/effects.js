...
import {
  state, tile,
  flyingArrows, magicEffects,
  clamp, playerAimAngle, currentWeapon,
  bowChargeProgress, bowShotStats, isBowWeapon, canUseWorldActions,
  getAttackEffect, getBowCharge
} from '../scenes/Game.js';

export function drawDashedLineGfx(gfx, x1, y1, x2, y2, dash = 8, gap = 7) {
  const dx = x2 - x1;
  const dy = y2 - y1;
...
export function drawArrowShapeGfx(gfx, x, y, angle, scale = 1, fillColor, strokeColor, strokeWidth = 1.5, strokeAlpha = 1) {
