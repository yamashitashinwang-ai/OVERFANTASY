...
import { state, tile, mapBounds, clamp } from '../scenes/Game.js';

export function getActorBody(actor) {
  if (!actor) return null;
  if (actor === state.player) return D.playerCircle?.body || null;
  const ed = D.entityDisplayMap.get(actor.id);
  if (ed?.circle?.body) return ed.circle.body;
  const pd = D.petDisplayMap.get(actor.id);
  if (pd?.circle?.body) return pd.circle.body;
  return null;
}

export function moveActor(actor, dx, dy, speed, dt) {
...
export function zeroAllVelocities() {
  ...
  if (D.playerCircle?.body) D.playerCircle.body.setVelocity(0, 0);
  for (const display of D.entityDisplayMap.values()) {
    if (display.circle?.body) display.circle.body.setVelocity(0, 0);
  }
...
export function teleportBody(actor) {
...
export function attachCircleBody(arc, radius, dynamic = true) {
  if (!D.pScene || arc.body) return arc.body;
  D.pScene.physics.add.existing(arc, !dynamic);
  arc.body.setCircle(radius);
  if (dynamic) {
    arc.body.setCollideWorldBounds(true);
    arc.body.setDrag(0, 0);
    arc.body.setBounce(0, 0);
    arc.body.setMaxVelocity(900, 900);
  }
  return arc.body;
}
