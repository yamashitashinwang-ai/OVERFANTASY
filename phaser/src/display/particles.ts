...
function ensureParticleTextures() {
  ...
  if (!tex.exists('p_dot')) {
    const g = D.pScene.make.graphics(graphicsConfig({ add: false }));
...
function scheduleEmitterCleanup(emitter: Phaser.GameObjects.Particles.ParticleEmitter, totalMs: number) {
  if (!D.pScene) return;
