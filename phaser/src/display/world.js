...
import { hexToInt, brightenColorInt } from './colors.js';
import { attachCircleBody, rebuildPhysicsForMap } from './physics.js';
import { ensureTileTextures } from './tiles.js';
import {
  state, tile,
  graveMaxDecay,
  clamp,
  mapBounds, currentPetScene, currentWeapon
} from '../scenes/Game.js';
...
export function rebuildDisplay() {
  ...
  D.playerCircle.body.reset(state.player.x * tile, state.player.y * tile);
...
export function syncObjectDisplay() {
  ...
  for (const o of state.objects) {
    ...
    if (!display) {
      ...
      display = { rect, labelBg, labelText, obj: o };
...
export function syncPetDisplay() {
  ...
  for (const [id, pet] of visiblePets) {
    ...
    if (pet.carried && display.circle.body) {
      display.circle.body.reset(pet.x * tile, pet.y * tile);
    }
